...
class RobustConnectionPool:
    ...
    def _start_health_check(self):
        ...
        # Start health check in background only if event loop is running
        try:
            loop = asyncio.get_running_loop()
            asyncio.create_task(health_check_loop())
            logger.debug("Health check task started")
        except RuntimeError:
            # No running event loop - health check will be started when needed
            logger.debug("No running event loop - skipping health check initialization")
...
class MemoryManager:
        ...
        try:
            ...
            with self.get_db_connection() as conn:
                ...
                """, (
                    ...
                    memory_content,
                    vector_str,
                    Json(metadata)  # Use Json adapter for JSONB type
                ))
    ...
    async def _check_chat_trigger(self, user_id: int, message_count: int = 0,
        ...
        """Check chat phase memory triggers - simplified to message count only"""
        # Only check message count trigger (every 3 messages)
    ...
    async def create_memory_from_conversations(self, user_id: int, memory_type: str,
        ...
        try:
            ...
            if conversation_ids:
                ...
                conversations = self.get_conversations_by_user(user_id, limit=10)
